...
function seed(db: import('better-sqlite3').Database): void {
  ...
  db.prepare(
    `INSERT INTO clubs (id, name, short_name, country_id, league_id, reputation, budget, wage_budget,
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(10, 'Buyer FC', 'BUY', 1, 1, 70, 10_000_000, 500_000, 'S', 20000, 3, 3, 3, '#1', '#2');
}
...
function insertFreeAgent(
  ...
  db.prepare(
    `INSERT INTO players (id, name, nationality, age, position, secondary_position, club_id, wage,
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(id, `FA ${id}`, 'X', age, 'ST', null, free ? null : 10, 0, 0, 5_000_000, 75, 75, 70, 90, 0, free ? 1 : 0);
}
...
describe('signFreeAgent', () => {
  it('signs when wage meets expectation', async () => {
    ...
    const res = await signFreeAgent(h, {
      playerId: 1,
      clubId: 10,
  ...
  it('rejects invalid contract lengths', async () => {
    ...
    const res1 = await signFreeAgent(h, {
      playerId: 1,
      clubId: 10,
      wageOffered: 50_000,
      contractYears: 0,
    ...
    const res2 = await signFreeAgent(h, {
      playerId: 1,
      clubId: 10,
      wageOffered: 50_000,
      contractYears: 6,

...
describe('players queries', () => {
  ...
  describe('getPlayersByClub', () => {
    it('returns 23-27 players for club 1', async () => {
      const players = await getPlayersByClub(db, 1, 1);
  ...
  describe('getPlayerById', () => {
    it('returns player with attributes for existing player', async () => {
      const result = await getPlayerById(db, 1, 1);
    ...
    it('returns null for non-existent player', async () => {
      const result = await getPlayerById(db, 1, 999999);
  ...
  describe('searchPlayers', () => {
    it('filters by position', async () => {
      const players = await searchPlayers(db, 1, { position: 'GK' });
    ...
    it('filters by age range', async () => {
      const players = await searchPlayers(db, 1, { minAge: 20, maxAge: 25 });
      expect(players.length).toBeGreaterThan(0);
      for (const p of players) {
    ...
    it('returns empty array when no players match', async () => {
      const players = await searchPlayers(db, 1, { minAge: 100 });
  ...
  describe('updatePlayerMorale', () => {
    it('changes morale of a player', async () => {
      const before = await getPlayerById(db, 1, 1);
      expect(before).not.toBeNull();

      const newMorale = before!.morale === 50 ? 60 : 50;
      await updatePlayerMorale(db, 1, 1, newMorale);

      const after = await getPlayerById(db, 1, 1);
    ...
    it('maps consecutiveLowMoraleWeeks from DB row', async () => {
      rawDb.prepare(
        `INSERT INTO players (id, save_id, name, nationality, age, position, club_id, wage,
         ...
         VALUES (88881, 1, 'Veteran A', 'English', 35, 'CM', 1, 5000, 2026, 500000, 65, 65, 40, 90, 0, 0, 3, 0)`,
      ).run();
      rawDb.prepare(
        `INSERT INTO player_attributes (player_id, save_id, finishing, passing, crossing, dribbling, heading,
         ...
         VALUES (88881, 1, 60,60,55,60,60,55,45,60,65,65,65,55,50,70,65,65,70,60)`,
      ).run();

      const p = await getPlayerById(db, 1, 88881);
    ...
    it('maps willRetireAtSeasonEnd = true when flag is 1', async () => {
      rawDb.prepare(
        `INSERT INTO players (id, save_id, name, nationality, age, position, club_id, wage,
          contract_end, market_value, base_potential, effective_potential, morale, fitness,
          injury_weeks_left, is_free_agent, consecutive_low_morale_weeks, will_retire_at_season_end)
         VALUES (88882, 1, 'Veteran B', 'English', 37, 'ST', 1, 5000, 2026, 500000, 65, 65, 30, 90, 0, 0, 5, 1)`,
      ).run();
      rawDb.prepare(
        `INSERT INTO player_attributes (player_id, save_id, finishing, passing, crossing, dribbling, heading,
          long_shots, free_kicks, vision, composure, decisions, positioning, aggression, leadership,
          pace, stamina, strength, agility, jumping)
         VALUES (88882, 1, 60,60,55,60,60,55,45,60,65,65,65,55,50,70,65,65,70,60)`,
      ).run();

      const p = await getPlayerById(db, 1, 88882);
      expect(p).not.toBeNull();
  ...
  describe('getPlayersAboutToRetire', () => {
    it('returns only players in given club with willRetireAtSeasonEnd = true', async () => {
      rawDb.prepare(
        `INSERT INTO players (id, save_id, name, nationality, age, position, club_id, wage,
          contract_end, market_value, base_potential, effective_potential, morale, fitness,
          injury_weeks_left, is_free_agent, consecutive_low_morale_weeks, will_retire_at_season_end)
         VALUES (88883, 1, 'Retiring Player', 'English', 36, 'LB', 1, 5000, 2026, 500000, 60, 60, 30, 90, 0, 0, 4, 1)`,
      ).run();
      rawDb.prepare(
        `INSERT INTO player_attributes (player_id, save_id, finishing, passing, crossing, dribbling, heading,
          long_shots, free_kicks, vision, composure, decisions, positioning, aggression, leadership,
          pace, stamina, strength, agility, jumping)
         VALUES (88883, 1, 55,60,65,55,60,50,45,58,60,62,60,55,50,65,65,65,65,60)`,
      ).run();

      const retiring = await getPlayersAboutToRetire(db, 1, 1);
    ...
    it('returns empty array when no player in club is retiring', async () => {
      const retiring = await getPlayersAboutToRetire(db, 1, 99999);
    ...
    it('returns only players with isFreeAgent = true', async () => {
      ...
      rawDb.prepare(
        `INSERT INTO players (id, save_id, name, nationality, age, position, secondary_position, club_id, wage,
         ...
         VALUES (99999, 1, 'Free Agent Player', 'English', 28, 'ST', NULL, NULL, 5000, 2026, 500000, 70, 70, 80, 90, 0, 1)`,
      ).run();
      rawDb.prepare(
        `INSERT INTO player_attributes (player_id, save_id, finishing, passing, crossing, dribbling, heading,
          long_shots, free_kicks, vision, composure, decisions, positioning, aggression, leadership,
          pace, stamina, strength, agility, jumping)
         VALUES (99999, 1, 70, 60, 55, 65, 60, 58, 45, 62, 70, 68, 72, 55, 50, 75, 70, 68, 72, 65)`,
      ).run();

      const freeAgents = await getFreeAgents(db, 1);

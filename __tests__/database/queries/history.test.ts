...
describe('history queries', () => {
  ...
  beforeEach(() => {
    ...
    rawDb.prepare(
      `INSERT INTO competitions (id, save_id, name, type, format, season, league_id)
       VALUES (1, 1, 'Premier League', 'league', 'round_robin', 1, 1),
              (2, 1, 'FA Cup',          'cup',    'knockout',    1, NULL)`,
    ).run();
    ...
    rawDb.prepare(
      `INSERT INTO season_competition_results (save_id, season, competition_id, champion_club_id, runner_up_club_id)
       VALUES (1, 1, 1, 1, 2), (1, 2, 1, 2, 1), (1, 1, 2, 3, 4)`,
    ).run();
    rawDb.prepare(
      `INSERT INTO season_relegated (save_id, season, league_id, club_id, final_position)
       VALUES (1, 1, 1, 10, 18), (1, 1, 1, 11, 19), (1, 1, 1, 12, 20)`,
    ).run();
    rawDb.prepare(
      `INSERT INTO season_awards (save_id, season, competition_id, award_type, rank, player_id, club_id, value)
       VALUES
         (1, 1, 1, 'top_scorer', 1, 100, 1, 25),
         (1, 1, 1, 'top_scorer', 2, 101, 2, 22),
         (1, 1, 1, 'mvp',        1, 100, 1, 8.4),
         (1, 2, 1, 'top_scorer', 1, 100, 2, 19)`,
    ).run();
    rawDb.prepare(
      `INSERT INTO season_player_titles (save_id, season, competition_id, club_id, player_id)
       VALUES (1, 1, 1, 1, 100), (1, 2, 1, 2, 100), (1, 1, 2, 3, 200)`,
    ).run();
  ...
  it('getSeasonSummary returns competitions with champion, runner-up, relegated, awards', async () => {
    const summary = await getSeasonSummary(db, 1, 1);
  ...
  it('getCompetitionHistory lists champions by season ascending', async () => {
    const history = await getCompetitionHistory(db, 1, 1);
  ...
  it('getClubTrophies aggregates titles and runner-ups per competition', async () => {
    const trophies = await getClubTrophies(db, 1, 1);
  ...
  it('getPlayerAwards returns all awards of a player chronologically', async () => {
    const awards = await getPlayerAwards(db, 1, 100);
  ...
  it('getPlayerTitles returns titles snapshotted for the player', async () => {
    const titles = await getPlayerTitles(db, 1, 100);

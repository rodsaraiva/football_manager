...
export function seedDatabase(db: DbHandle, data: SeedData): void {
  // FK cycle: save_games.player_club_id -> clubs.id  AND  clubs.save_id -> save_games.id.
  // Resolution: insert save_games with FK off (forward ref to clubs), then re-enable FK so
  // that the rest of the data inserts are verified normally (country_id, league_id, etc.).
  // PRAGMA foreign_keys cannot change inside a transaction, so the save_games bootstrap
  // happens outside the main transaction, then we open a second transaction for all data.
  db.exec('PRAGMA foreign_keys = OFF');
  db.prepare(
    "INSERT INTO save_games (id, name, current_season, current_week, player_club_id, difficulty, board_trust, created_at, updated_at) VALUES (1, 'Seed', 1, 1, 1, 'normal', 50, '', '')",
  ).run();
  ...
  try {
    ...
    for (const c of data.clubs) insertClub.run(c.id, 1, c.name, c.shortName, c.countryId, c.leagueId, c.reputation, c.budget, c.wageBudget, c.stadiumName, c.stadiumCapacity, c.trainingFacilities, c.youthAcademy, c.medicalDepartment, c.primaryColor, c.secondaryColor);
    ...
    for (const p of data.players) insertPlayer.run(p.id, 1, p.name, p.nationality, p.age, p.position, p.secondaryPosition, p.clubId, p.wage, p.contractEnd, p.marketValue, p.basePotential, p.effectivePotential, p.morale, p.fitness, p.injuryWeeksLeft, p.isFreeAgent ? 1 : 0, p.preferredFoot, p.weakFootAbility);
    ...
    for (const a of data.playerAttributes) insertAttrs.run(a.playerId, 1, a.finishing, a.passing, a.crossing, a.dribbling, a.heading, a.longShots, a.freeKicks, a.vision, a.composure, a.decisions, a.positioning, a.aggression, a.leadership, a.pace, a.stamina, a.strength, a.agility, a.jumping);

    // Insert staff
    const insertStaff = db.prepare('INSERT INTO staff (id, save_id, name, role, club_id, ability, wage, contract_end) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    for (const s of data.staff) insertStaff.run(s.id, 1, s.name, s.role, s.clubId, s.ability, s.wage, s.contractEnd);
    ...
    for (const t of data.tactics) insertTactic.run(t.id, 1, t.clubId, t.name, t.isActive ? 1 : 0, t.formation, t.mentality, t.pressing, t.passingStyle, t.tempo, t.width);
